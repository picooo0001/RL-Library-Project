...
class App:
    ...
    def __init__(self):
        ...
        logo_image = logo_image.resize(
            (logo_width, logo_height), Image.ANTIALIAS
        )  # resizes the logo photo
        logo_image = ImageTk.PhotoImage(logo_image)
        self.root.iconphoto(True, logo_image)  # changes the logo

        background_label = tk.Label(
            self.root, bg="black", image=bg_photo
        )  # creates a label for our bg photo
        background_label.place(
            x=0, y=0, relwidth=1, relheight=1
        )  # places the label at x = 0, y = 0
        background_label.image = (
            bg_photo  # sets the background label as the root window's background
        )

        self.output_artist_var = (
            StringVar()
        )  # creates a StringVar for the first search (artists)
        self.output_headliner_var = (
            StringVar()
        )  # creates a StringVar for the second search (headliner)
        self.time_var = StringVar()  # creates a StringVar for the third search (time)
        self.ticket_var = (
            StringVar()
        )  # creates a StringVar for the first search in the toplevel window (ticket)

        self.day_check_var = ctk.StringVar(
            value="False"
        )  # creates more StringVars for all the different checkboxes in the toplevel window
    ...
    def go_button_artist(self):
        ...
        current_entry = (
            self.artist_entry.get()
        )  # if go is pressed, the current input is saved in `current_entry`
        for i in current_entry:  # the entry gets deleted
            self.artist_entry.delete(0)
        output = self.artists_instance.search_artist(
            user_input=current_entry
        )  # executes the 'search_artists' method from Artists and saves the result in `output`
        self.output_artist_var.set(
            output
        )  # sets the StringVar from above to `output` (result of the search)
    ...
    def find_ticket_button(self):
        ...
        single_day_bool = (
            self.day_check_var.get()
        )  # when the search button (toplevel window) is pressed, this method gets if a checkbox is ticked or not
    ...
    def textbox_1(self):
        ...
        textbox.insert(
            "0.0", "Search your artist:"
        )  # inserts text at line 0 character 0
    ...
    def second_window(self):
        ...
        background_label = tk.Label(
            self.second_window, bg="black", image=toplevel_bg_photo
        )  # creates a label for our bg photo
        background_label.place(
            x=0, y=0, relwidth=1, relheight=1
        )  # places the label at x = 0, y = 0
        ...
        merch_checkbox = ctk.CTkCheckBox(
            self.second_window,
        ...
        fastlane_checkbox = ctk.CTkCheckBox(
            self.second_window,
            text="Fast lane?",
            corner_radius=10,
            height=40,
        ...
        septoilets_checkbox = ctk.CTkCheckBox(
            self.second_window,
        ...
        viparea_checkbox = ctk.CTkCheckBox(
            self.second_window,
            text="Vip Area?",
            corner_radius=10,
            height=40,
        ...
        below100_checkbox = ctk.CTkCheckBox(
            self.second_window,
            text="Below 100€?",
            corner_radius=10,
            height=40,
        ...
        below200_checkbox = ctk.CTkCheckBox(
            self.second_window,
            text="Below 200€?",
            corner_radius=10,
            height=40,
        ...
        below300_checkbox = ctk.CTkCheckBox(
            self.second_window,
            text="Below 300€?",
            corner_radius=10,
            height=40,
