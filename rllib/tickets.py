...
class Tickets:
    ...
    def catch_userinput(self, single_day, incl_merch, fast_lane, sep_toilets, vip_area, below_100, below_200, below_300):
    ...
    def search_tickets(self):
        ...
        #counts the number of matches between the ticket's values and the user's search decisions
            self.match_count = sum(search_value == ticket[i + 1] for i, search_value in enumerate(self.search))
            if self.match_count == 8:  #exact match
                self.exact_matches.append(ticket[0])  #first Index of tuple is given to exact_matches
            elif self.match_count > self.max_match_count:  #partial match with most matches so far
                self.partial_matches_first_point = [ticket[0]]  #re-initialized as a list containing the identifier of the current ticket
                self.max_match_count = (self.match_count)  #max_match_count is updated to the new match_count.
            elif (self.match_count == self.max_match_count):  #additional partial match with the same match count
                self.partial_matches_first_point.append(ticket[0])

        #returns the results
        if self.exact_matches:
            output = ("Your exact match ticket(s): \n"  + "\n".join(self.exact_matches)) # \n  is used to make sure that the output fits into the GUI Label
        elif self.partial_matches_first_point:
            output = ("Tickets with the most matches:\n" + "\n".join(self.partial_matches_first_point))
        else:
