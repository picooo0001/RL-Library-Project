...
class Artists:
    ...
    def __init__(self):
        ...
        next(
            reader
        )  # header line will be skipped, that only the important data will be written in file
    ...
    def __len__(
        self,
    ):  # not very usefull, implemented before we had a proper plan, same goes for __getitem__
    ...
    def search_artist(self, user_input):
        ...
        for artist in self.artists:  # loops over the tuple artists
            ...
            if user_input in formatted_artist:
                index = formatted_artist.index(
                    user_input
                )  # finds index to search in the other tuples
                headliner = self.headliner[index]
                ...
                date = self.date[index]
                output = ", ".join(
                    [artist, stage, time, date]
                )  # saves the found values together in `output`
    ...
    def search_headliner(self):
        ...
        if user_input == "true":
            ...
            if found_indices:
                output = "\n".join(
                    [
                        f"{self.artists[index]}, {self.time[index]}, {self.date[index]}"
                        for index in found_indices
                    ]
                )  # searches the index in other collums
            else:
                output = "Sorry! We couldn't find what you were looking for!"
        else:
    ...
    def search_stage(self):  # not very important (not implemented)
        ...
        while not found:
            ...
            for index in found_indices:
                output = ", ".join(
                    [self.artists[index], self.date[index], self.time[index]]
                )
                found = True
    ...
    def search_time(self, user_input):  # same logic as above
        ...
        while not found:
            ...
            for index in found_indices:
                output += (
                    ", ".join(
                        [self.date[index], self.artists[index], self.stage[index]]
                    )
                    + "\n"
                )
                found = True
